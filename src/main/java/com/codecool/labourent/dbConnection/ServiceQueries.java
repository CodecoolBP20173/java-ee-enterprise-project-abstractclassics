...
public class ServiceQueries {
    ...
    public static List<Service> getAllRecordsFromTable(String column, String ascOrDesc){
        ...
        serviceList =  em.createQuery("SELECT allRecords from Service  allRecords ORDER BY " + validateColumnName(column) + " " +  validateSortDirection(ascOrDesc)).getResultList();
        return serviceList;
    }
    ...
    private static  List<String> assignColumnNames(EntityManager em) {
        if (columnNamesArray == null) {
            columnNamesArray =  em.createNativeQuery ("select column_name from information_schema.columns where table_name='service'").getResultList();
